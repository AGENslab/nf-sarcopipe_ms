...
# ------------------------------------------------------------
# PCA of miRNA expression using variance-stabilized counts.
# Visualizes global expression structure across samples
# including confidence ellipses per condition.
# ------------------------------------------------------------
...
if (length(args) < 3) {
  stop("Usage: Rscript plot_pca_mirna_expression.R <counts.tsv> <sample_info.tsv> <out_prefix>")
}
...
if (!all(required_cols %in% colnames(meta_df))) {
  stop("Metadata file must contain columns: sample and condition")
}
...
if (length(common_samples) < 2) {
  stop("Fewer than 2 shared samples between counts matrix and metadata")
}
...
if (!identical(colnames(counts_mat), rownames(meta_df))) {
  stop("Sample order mismatch between counts matrix and metadata")
}
...
if (nrow(dds) < 2) {
  stop("Too few miRNAs remaining after filtering")
}
