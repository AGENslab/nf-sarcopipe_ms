...
############################################################
# Figure S3D – Linear model corrected by PC1, PC2 and PC3
#
# Description:
# Runs a gene-wise linear model on variance-stabilized mRNA-seq
# expression values while correcting for PC1, PC2 and PC3.
#
# The script:
...
#
# Outputs:
# - DEG_PC1_PC2_PC3_corrected.csv
#
# Usage:
# Rscript FigureS3D_model_PC1_PC2_PC3.R \
#   --base_dir /path/to/star_salmon \
#   --samplesheet metadata.tsv \
#   --out_csv DEG_PC1_PC2_PC3_corrected.csv
############################################################
...
for (i in seq_len(nrow(vsd_mat))) {
  ...
  model <- lm(y ~ group + PC1 + PC2 + PC3, data = meta)
